...
class SpecialController extends Controller
{
    private function queryWithDetails()
    {
        ...
        return DB::table('property')
            ...
            ->selectRaw('property.*, property.media, ' . $detailCols);
    }

    public function index(Request $request)
    {
        // Get property IDs with active nardban
        $boostedPropertyIds = PropertyBoost::active()->pluck('property_id')->unique()->toArray();

        $query = $this->queryWithDetails();

        // Only show boosted properties
        $query->whereIn('property.id', $boostedPropertyIds);
        ...
        if ($priceMin) {
            $query->where('property_details.price', '>=', preg_replace('/\D/', '', $priceMin));
        }

        $priceMax = $request->get('price_max');
        if ($priceMax) {
            $query->where('property_details.price', '<=', preg_replace('/\D/', '', $priceMax));
        }

        // ── Rent ──
        $rentMin = $request->get('rent_min');
        if ($rentMin) {
            $query->where('property_details.rent', '>=', preg_replace('/\D/', '', $rentMin));
        }

        $rentMax = $request->get('rent_max');
        if ($rentMax) {
            $query->where('property_details.rent', '<=', preg_replace('/\D/', '', $rentMax));
        }
        ...
        switch ($sort) {
            case 'price_asc':
                $query->orderBy('property_details.price', 'ASC');
                break;
            case 'price_desc':
                $query->orderBy('property_details.price', 'DESC');
                break;
            case 'area_desc':
        ...
        if ($request->ajax() || $request->has('ajax')) {
            $html = view('special.partials.cards', compact('properties'))->render();
            $pagination = view('special.partials.pagination', compact('properties'))->render();
        ...
        return view('special.index', compact('properties', 'cities', 'user', 'favoriteIds'));
    }
}
